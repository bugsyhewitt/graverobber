...
func TestJSONLWriter_OmitsEmptyVectorFields(t *testing.T) {
	...
	got := buf.String()
	for _, leaked := range []string{"mx_hosts", "dkim_selector", "dmarc_uri", "spf_include", "spf_lookups", "nameservers", "dnssec_weak_algs", "ds_key_tags"} {
...
func TestTerminalWriter_RendersDetailForEveryVector(t *testing.T) {
	cases := []struct {
		...
		},
		{
			name: "dnssec",
			f: finding.Finding{
				Subdomain: "example.com", Vector: finding.VectorDNSSEC,
				Confidence: finding.Potential, DSKeyTags: []uint16{12345},
			},
			wantSub: []string{"example.com", "12345", "dnssec"},
		},
		{
			name: "dnssec-weak-algorithm",
			f: finding.Finding{
				Subdomain: "example.com", Vector: finding.VectorDNSSEC,
				Confidence:     finding.Potential,
				DSKeyTags:      []uint16{12345},
				DNSSECWeakAlgs: []string{"RSASHA1"},
			},
			wantSub: []string{"example.com", "RSASHA1", "dnssec", "weak"},
		},
		{
			name: "tlsrpt",
			f: finding.Finding{
				Subdomain: "example.com", Vector: finding.VectorTLSRPT,
				Confidence: finding.Potential, Service: "_smtp._tls.example.com",
				TLSRPTURIHost: "reports.gone.net",
			},
			wantSub: []string{"example.com", "reports.gone.net", "tlsrpt"},
		},
		{
			name: "autodiscover",
			f: finding.Finding{
				Subdomain: "example.com", Vector: finding.VectorAutodiscover,
				Confidence: finding.Potential, Service: "autodiscover.example.com",
				CNAME: "example.mail.dead-tenant.invalid",
			},
			wantSub: []string{"example.com", "autodiscover.example.com", "autodiscover"},
		},
	}
